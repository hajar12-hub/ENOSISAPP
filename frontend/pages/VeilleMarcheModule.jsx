...
function LegacyWizard({ user }) {
  const { visitId } = useParams(); const navigate = useNavigate(); const [step, setStep] = useState('secteur'); const [options, setOptions] = useState({ secteurs: [], villes: [], categories: [], subcategories: [] }); const [form, setForm] = useState({ secteur: '', ville: '', magasin: '' }); const [record, setRecord] = useState(null); const [store, setStore] = useState(null); const [currentCategory, setCurrentCategory] = useState(null); const [currentSubcategory, setCurrentSubcategory] = useState(null); const [brands, setBrands] = useState([]); const [entries, setEntries] = useState({}); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState('')
  const hydrateBrands = useCallback(async (sub) => { const result = await api.marques({ sous_categorie: sub.id }); const hydrated = await Promise.all(result.map(async (brand) => ({ ...brand, segment_nom: brand.segment_nom || '', skus: await api.skus(brand.id) }))); setBrands(hydrated) }, [])
  useEffect(() => { let alive = true; Promise.all([api.secteurs(), api.villes(), api.categories()]).then(async ([secteurs, villes, categories]) => { if (!alive) return; setOptions({ secteurs, villes, categories, subcategories: [] }); if (visitId) { const [visit, releves] = await Promise.all([api.visite(visitId), api.releves(visitId)]); const magasin = await api.magasin(visit.magasin); if (!alive) return; setRecord(visit); setStore(magasin); setForm({ secteur: magasin.secteur, ville: magasin.ville, magasin: magasin.nom || '' }); setEntries(Object.fromEntries(releves.map((row) => [row.sku, row]))); setStep(STEPS.includes(visit.etape) ? visit.etape : 'recap') } }).catch((e) => alive && setError(apiMessage(e))).finally(() => alive && setLoading(false)); return () => { alive = false } }, [visitId])
  const go = (next) => { setError(''); setStep(next) }
  const createRecord = async () => { if (record) return go('categorie'); if (!form.magasin.trim()) return setError('Veuillez renseigner le nom du magasin.'); setSaving(true); try { const magasin = await api.createMagasin({ secteur: form.secteur, ville: form.ville, nom: form.magasin.trim() }); const visit = await api.createVisite(magasin.id); setStore(magasin); setRecord(visit); go('categorie') } catch (e) { setError(apiMessage(e)) } finally { setSaving(false) } }
  const selectCategory = async (category) => { setCurrentCategory(category); setSaving(true); try { const subs = await api.sousCategories(category.id); setOptions((current) => ({ ...current, subcategories: [...current.subcategories.filter((item) => String(item.categorie) !== String(category.id)), ...subs] })); go('sous_categorie') } catch (e) { setError(apiMessage(e)) } finally { setSaving(false) } }
  const selectSubcategory = async (sub) => { setCurrentSubcategory(sub); setSaving(true); try { await hydrateBrands(sub); if (record) await api.updateVisite(record.id, { etape: 'sku' }); go('sku') } catch (e) { setError(apiMessage(e)) } finally { setSaving(false) } }
  const payload = () => Object.values(entries).filter((row) => row.prix_conso_ttc && row.prix_detail_ttc).map(({ sku, prix_conso_ttc, prix_detail_ttc }) => ({ sku, prix_conso_ttc, prix_detail_ttc }))
  const incomplete = useMemo(() => { const valid = Object.values(entries).filter((row) => Number(row.prix_conso_ttc) > 0 && Number(row.prix_detail_ttc) > 0); return options.subcategories.find((sub) => !valid.some((row) => String(row.sous_categorie) === String(sub.id) || row.sous_categorie_nom === sub.nom)) }, [entries, options.subcategories])
  const persist = async (submit = false) => { if (!record) return; if (submit) { if (Object.values(entries).some((row) => !Number(row.prix_conso_ttc) || !Number(row.prix_detail_ttc))) return setError('Veuillez compléter les deux prix pour chaque SKU sélectionné.'); if (incomplete) return setError(`Veuillez compléter la sous-catégorie « ${incomplete.nom} ».`) } setSaving(true); try { await api.saveReleves(record.id, payload(), !submit); if (submit) navigate('/veille-marche/historique', { state: { successMessage: 'Relevé soumis avec succès.' } }); else navigate('/veille-marche/brouillons', { state: { successMessage: 'Brouillon sauvegardé avec succès.' } }) } catch (e) { setError(apiMessage(e)) } finally { setSaving(false) } }
  const nextPending = async () => { if (!record) return; setSaving(true); try { await api.saveReleves(record.id, payload(), true); const next = incomplete; if (!next) { await api.updateVisite(record.id, { etape: 'recap' }); return go('recap') } const category = options.categories.find((item) => String(item.id) === String(next.categorie)); setCurrentCategory(category); setCurrentSubcategory(next); await hydrateBrands(next); await api.updateVisite(record.id, { etape: 'sku' }); go('sku') } catch (e) { setError(apiMessage(e)) } finally { setSaving(false) } }
  const back = () => { const index = STEPS.indexOf(step); if (index) go(STEPS[index - 1]) }
  const continueStep = () => { if (step === 'secteur' && form.secteur) go('ville'); else if (step === 'ville' && form.ville) go('magasin'); else if (step === 'magasin') createRecord(); else if (step === 'sku') go('recap'); else setError('Veuillez effectuer une sélection avant de continuer.') }
  if (loading) return <Loader/>; const selectedCategorySubs = currentCategory ? options.subcategories.filter((item) => String(item.categorie) === String(currentCategory.id)) : []
  return <><PageHeader title="Nouveau relevé"/>{error && <div className="alert alert--error">{error}</div>}<section className="wizard-card"><WizardProgress current={step}/><div className="wizard-body">{step === 'secteur' && <FieldStep title="Sélectionnez votre secteur" text="Cette information sera conservée pour tout le relevé." fieldLabel="Secteur" value={form.secteur} items={options.secteurs} onChange={(value) => setForm((old) => ({ ...old, secteur: value }))}/>} {step === 'ville' && <FieldStep title="Sélectionnez votre ville" text="Choisissez la ville du point de vente visité." fieldLabel="Ville" value={form.ville} items={options.villes} onChange={(value) => setForm((old) => ({ ...old, ville: value }))}/>} {step === 'magasin' && <div className="visit-step"><div className="step-heading"><span className="step-heading__icon">⌂</span><div><h2>Quel magasin visitez-vous ?</h2><p>La date de relevé est créée automatiquement par le serveur.</p></div></div><label className="select-card"><span>Nom du magasin</span><input value={form.magasin} placeholder="Ex. Marjane Casablanca" onChange={(e) => setForm((old) => ({ ...old, magasin: e.target.value }))}/></label></div>} {step === 'categorie' && <SelectionCards items={options.categories} onSelect={selectCategory}/>} {step === 'sous_categorie' && <SelectionCards items={selectedCategorySubs} onSelect={selectSubcategory} icon="◇" empty="Aucune sous-catégorie pour cette catégorie."/>} {step === 'sku' && <SkuFields brands={brands} entries={entries} setEntries={setEntries} category={currentCategory} subcategory={currentSubcategory}/>} {step === 'recap' && <Recap record={record} store={store} selections={{ secteur_nom: label(options.secteurs, form.secteur), ville_nom: label(options.villes, form.ville), magasin_nom: form.magasin }} entries={entries} categories={options.categories} subcategories={options.subcategories} user={user}/>}</div>{step === 'recap' ? <div className="wizard-actions wizard-actions--recap"><Button variant="secondary" onClick={back} disabled={saving}>Retour</Button><Button variant="secondary" onClick={() => persist(false)} disabled={saving}>Sauvegarder le brouillon</Button><Button onClick={() => persist(true)} disabled={saving}>Soumettre</Button><Button variant="secondary" onClick={nextPending} disabled={saving}>Passer à la catégorie suivante</Button></div> : <div className="wizard-actions"><Button variant="secondary" onClick={back} disabled={saving} style={{ visibility: step === 'secteur' ? 'hidden' : 'visible' }}>Retour</Button>{!['categorie', 'sous_categorie'].includes(step) && <Button onClick={continueStep} disabled={saving}>{saving ? 'Enregistrement…' : 'Continuer'}</Button>}</div>}</section></> }
